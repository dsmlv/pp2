            ...
            if pause:
                ...
                if exist:
                    if SCORE > high_score:
                        ...
                        '''
                        cursor.execute(update_sc, (SCORE,name))
                    if level > lvl:
                        update_l = '''
                            UPDATE snake SET level = %s WHERE username = %s;
                        '''
                        cursor.execute(update_l, (level,name))
                #otherwise inserting
                else:
                    cursor.execute(insert_int, (f'{name}', f'{SCORE}', f'{level}'))
        #     cursor.close()
        #     conn.commit()
        #     conn.close()
        pygame.display.flip()
        ...
        while lose or win:
            ...
            if lose:
                ...
                if exist:
                    if SCORE > high_score:
                        update_s = '''
                            UPDATE snake SET highscore = %s WHERE username = %s;
                        '''
                        cursor.execute(update_s,(SCORE,name))
                    if level > lvl:
                        update_l = '''
                            UPDATE snake SET level = %s WHERE username = %s;
                        '''
                ...
                #otherwise inserting
                    ...
                    cursor.execute(insert_into, (f'{name}', f'{SCORE}', f'{level}'))
                cursor.close()
                conn.commit()
                conn.close()
